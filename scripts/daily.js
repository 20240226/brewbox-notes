...
async function main() {
  ...
  const img = await sharp({ create: { width: 1242, height: 1660, channels: 4, background: { r: 28, g: 21, b: 18, alpha: 1 } } })
    ...
    .png().toBuffer();
  fs.writeFileSync(path.join(NOTES_DIR, 'cover.png'), img);
  console.log('封面已生成');
  ...
  const html = [
    ...
    '.cover{width:372px;margin:16px auto;border-radius:14px;overflow:hidden;box-shadow:0 12px 48px rgba(0,0,0,0.6)}',
    ...
    '.card p{color:#C8B8A8;font-size:14px;line-height:1.9}',
    '.tags{margin-top:16px;display:flex;gap:8px;flex-wrap:wrap}',
    '.tags span{background:rgba(200,149,108,0.15);color:#C8956C;padding:4px 12px;border-radius:12px;font-size:12px}',
    'a{color:#C8956C;text-decoration:none;font-size:14px}',
    'a:hover{text-decoration:underline}',
    '</style></head><body>',
    '<h1>' + title + '</h1>',
    '<div class="cover"><img src="cover.png" alt="封面"></div>',
    '<div>',
    '<button class="btn" onclick="var a=document.createElement(\'a\');a.download=\'cover.png\';a.href=\'cover.png\';a.click()">保存封面</button>',
    '<button class="btn" onclick="navigator.clipboard.writeText(\'' + title + '\\n\\n' + note.body.replace(/'/g, "\\'").replace(/"/g, '\\"') + '\\n\\n' + (note.tags || []).map(function(t) { return '#' + t; }).join(' ') + '\').then(function(){this.textContent=\'已复制\';}.bind(this))">复制文案</button>',
    '</div>',
